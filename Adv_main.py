...
if __name__ == '__main__':
    ...
    print("Successfully load the trained ego agent")
    ...
    if TRAIN:
        ...
        for episode in range(MAX_TRAIN_EPISODE):
            ...
            if episode % SAVING_PER_EPISODE == 0 and episode != 0:
                bv_model.save(model_name="DQN-Ego")
            print(f"Episode: {episode + 1}, Reward: {episode_reward:.2f}")
        env.close()
        writer.close()

    if TEST:
        env = RecordVideo(env, video_folder="BV_model/DQN-Ego/videos", episode_trigger=lambda e: True)
        env.unwrapped.set_record_video_wrapper(env)
        env.configure({"simulation_frequency": 15})  # Higher FPS for rendering
        # load the trained bv_model
        bv_model.load(model_name="DQN-Ego")
